abort "Please use Ruby 1.9 to build SampleProject!" if RUBY_VERSION !~ /^1\.9/
...
# Create a compile task for an SampleProject package. This task will compute
...
namespace :sampleproject do
  %w(core).each do |package|
    task package => compile_package_task("sampleproject-#{package}", "sampleproject-#{package}")
  end
end

# Create a build task that depends on all of the package dependencies
task :build => ["sampleproject:core"]

distributions = {
  "sampleproject" => ["sampleproject-core"]
}
...
namespace :release do
  ...
  namespace :framework do
    ...
    task :changelog do
      ...
      changes = `#{cmd}`
      output = "*SamplePackage #{PROJECT_VERSION} (#{Time.now.strftime("%B %d, %Y")})*\n\n#{changes}\n"
    ...
    task :tag do
      puts "Tagging v#{PROJECT_VERSION}"
      system "git tag v#{PROJECT_VERSION}" unless pretend?
    end
